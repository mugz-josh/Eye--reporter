...
export const upvotesController = {
      ...
      const query = `
        SELECT COUNT(*) as count,
               CASE WHEN EXISTS(
                 ...
                 WHERE report_type = $1 AND report_id = $2 AND user_id = $3
               ) THEN 1 ELSE 0 END as user_upvoted
        FROM upvotes
        WHERE report_type = $4 AND report_id = $5
      `;

      const userId = req.user?.id || null;
      const result = await pool.query(query, [reportType, reportId, userId, reportType, reportId]);
  ...
  toggleUpvote: async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const checkResult = await pool.query(checkQuery, [reportId]);
      ...
      `;
      const upvoteResult = await pool.query(checkUpvoteQuery, [userId, reportType, reportId]);
