...
export const commentsController = {
  ...
  getComments: async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const result = await pool.query(query, [reportType, reportId]);
  ...
  addComment: async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const commentResult = await pool.query(selectQuery, [insertResult.rows[0].id]);

      sendSuccess(res, 201, commentResult.rows[0]);
    } catch (err) {
      sendError(res, 500, "Database error", err);
    }
  },
