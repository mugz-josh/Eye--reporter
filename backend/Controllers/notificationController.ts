...
createNotificationForUser: async (payload: {
  ...
  getUserNotifications: async (
    ...
    try {
      ...
      res
        .status(500)
        .json({ status: 500, error: "Failed to fetch notifications" });
    }
  },
  ...
  markAllAsRead: async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      res
        .status(500)
        .json({ status: 500, error: "Failed to mark notifications as read" });
    }
  },
  ...
  deleteNotification: async (
    ...
    try {
      ...
      res
        .status(500)
        .json({ status: 500, error: "Failed to delete notification" });
    }
  },
  deleteOldNotifications: async (daysOld: number = 30): Promise<void> => {
    try {
      ...
      console.log(
        `Deleted ${result.affectedRows} notifications older than ${daysOld} days`
      );
    } catch (err) {
